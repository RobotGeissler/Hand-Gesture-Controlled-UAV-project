...
class SensorDataset(Dataset):
    def __init__(self, data_dir, time_window=1, include_augmented=True):
        """
        SensorDataset that handles original and augmented data.

        Args:
        - data_dir (str): Directory containing gesture folders.
        - time_window (int): Time window for creating data points.
        - include_augmented (bool): Whether to include augmented data.
        """
...
def objective(trial):
    ...
    model.to("cuda")
    # Training
    logger = TensorBoardLogger("tb_logs", name="optuna_run_augmented")
    trainer = Trainer(
        strategy='ddp',
        devices=1,
        max_epochs=10,
...
if (False):
    ...
    }, "best_model_data_aug.pth")

# get model params
checkpoint = torch.load("best_model_data_aug.pth")
